	...
	unsigned char* k, unsigned char* v)
{
	...
	int wr = 0;
	int outlen = 0;
	unsigned char uname[256] = {0};
	unsigned int salt[32] = {0};
	unsigned int pkey[32] = {0};
	unsigned int outbuf[SIZE + AES_BLOCK_SIZE] = {0};
	unsigned int md[SHA256_DIGEST_LENGTH] = {0};
	unsigned char psalt[64] = {0};
	unsigned char salt_phash[32] = {0};
	...
	if ((ks = open(path,( O_CREAT | O_RDWR | O_TRUNC),
		(S_IREAD | S_IWRITE))) == -1)
	{
		perror("\n ERROR,OPEN()::");
		return 1;
	...
	if (!(PKCS5_PBKDF2_HMAC_SHA1(pwd, pwd_len, salt, 32, itr, 32, pkey)))
	{
		perror("\n ERROR,PBKG2::");
		return 1;
	}
	...
	{
		dbug_p("ERROR,PSALT LAST NULL_CKS\n");
		return 1;
	}
	...
	//Generating Password hash where pwd = input pwd+salt
	if (!SHA256_Init(&sh))
	{
		perror("\n ERROR,SHA256_CKS::");
		return 1;
	...
	// 1 item in KS:: Writing Pwd Hash to KS (32 Bytes)
	if ((wr = write(ks, md, SHA256_DIGEST_LENGTH))
		!= SHA256_DIGEST_LENGTH)
	{
		dbug_p("ERROR,WRITE_SHA256_CKS,bytes written:%d:\n",wr);
		return 1;
	}
	// 2 Item in KS:: SALT used for Pwd HASH (32Bytes)
	if ((wr = write(ks, salt_phash, 32)) != 32)
	{
		if (wr == 0)
		{
			dbug_p("PSALT_ITEM 2:ZERO BYTES written \n");
		}
		else
		{
			if (wr < 0)
	...
	// 3 Item in KS:: SALT used to gen session key 'S'(32Bytes)
	if ((wr = write(ks, salt, 32)) != 32)
	{
		if (wr == 0)
		{
			dbug_p("ZERO BYTES written \n");
		}
		else
		{
			if (wr < 0)
	...
	if ((wr = write(ks,v,32)) != 32)
	{
		if (wr == 0)
		{
			dbug_p("V_CKS_ZERO BYTES written \n");
		}
		else
		{
			if (wr < 0)
	...
	// 5(Last) Item in KS:: Key final (48 Bytes)
	if (write(ks, outbuf, outlen) != outlen)
	{
	...
	unsigned char* k, unsigned char* v)
{
	...
	unsigned char r_phash[SHA256_DIGEST_LENGTH] = {0};
	unsigned char salt_s[32] = {0};
	unsigned char salt_h[32] = {0};
	unsigned char psalt[64] = {0};
	unsigned char key_e[48] = {0};
	unsigned char outbuf[SIZE + AES_BLOCK_SIZE] = {0};
	char path[256]= "/home/";
	...
	rd = 0;
	if ((rd = read(ks, r_phash, SHA256_DIGEST_LENGTH))
		!= SHA256_DIGEST_LENGTH)
	{
	...
	if ( j != 32) // if hash does not match return an ERROR
	{
	...
	if (!(PKCS5_PBKDF2_HMAC_SHA1(pwd, pwd_len,
		salt_s, 32, itr, 32, s_key)))
	{
		perror("\n ERROR,PBKG2_RKS::");
		return 1;
	}
	...
	if (!(EVP_DecryptInit_ex(&dp, EVP_aes_256_cbc(), NULL,
		s_key, salt_s)))
	{
